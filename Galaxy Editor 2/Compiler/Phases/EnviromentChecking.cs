...
namespace Galaxy_Editor_2.Compiler.Phases
{
    ...
    {
        private static ResourceManager LocRM = new ResourceManager("Galaxy_Editor_2.Language", typeof(Language).Assembly);
        ...
        private class ProcessActionBlock : DepthFirstAdapter
        {
            ...
            public override void  CaseAValueReturnStm(AValueReturnStm node)
            {
                ...
                errors.Add(new ErrorCollection.Error(node.GetToken(), Util.GetAncestor<AASourceFile>(node), LocRM.GetString("ErrorText18")));
            }
        }
                ...
                for (int i = stringObfuscationErrors.Count - 1; i >= 0; i--)
                {
                    ...
                    else
                    {
                        children.Add(new ErrorCollection.Error(stringObfuscationErrors[i].GetStringLiteral(), Util.GetAncestor<AASourceFile>(stringObfuscationErrors[i]),
                                                                               LocRM.GetString("ErrorText20"), true));
                    }
                }
            }
            ...
            foreach (AInitializerDecl initializer in initializers)
            {
                ...
                for (int i = 0; i < initializer.GetInitializerParam().Count; i++)
                {
                    ...
                    if (param is ALibraryNameInitializerParam)
                    {
                        ...
                        {
                            errors.Add(new ErrorCollection.Error(aParam.GetToken(), LocRM.GetString("ErrorText23")));
                        }
                        if (strings.Length == 0 || strings[0] == "")
                        {
                            errors.Add(new ErrorCollection.Error(aParam.GetToken(), LocRM.GetString("ErrorText25")));
                            continue;
                        }
                    ...
                    if (param is ALibraryVersionInitializerParam)
                    {
                        ...
                        {
                            errors.Add(new ErrorCollection.Error(aParam.GetToken(), LocRM.GetString("ErrorText24")));
                        }
                        if (strings.Length == 0 || strings[0] == "")
                        {
                            errors.Add(new ErrorCollection.Error(aParam.GetToken(), LocRM.GetString("ErrorText26")));
                            continue;
                        }
                    ...
                    if (param is ARequiredLibrariesInitializerParam)
                    {
                        ...
                        foreach (string s in strings)
                        {
                            ...
                            if (!s.Contains(":"))
                            {
                                errors.Add(new ErrorCollection.Error(aParam.GetToken(), LocRM.GetString("ErrorText27")));
                                continue;
                            }
            ...
            foreach (KeyValuePair<AInitializerDecl, List<KeyValuePair<string, string>>> requiredLibraryList in requiredLibraries)
            {
                for (int i = 0; i < requiredLibraryList.Value.Count; i++)
                {
                    ...
                    if (foundLib)
                        continue;
                    //Didn't find the lib
                    errors.Add(new ErrorCollection.Error(requiredLibraryList.Key.GetToken(), LocRM.GetString("ErrorText28") + requiredLibrary.Key + ":" + requiredLibrary.Value + LocRM.GetString("ErrorText29")));
        ...
        private void LocateCycles(Dictionary<AInitializerDecl, List<AInitializerDecl>> dependancies, Dictionary<KeyValuePair<AInitializerDecl, AInitializerDecl>, int> cycleElementCount, List<AInitializerDecl> visited, List<AInitializerDecl> path, AInitializerDecl current)
        {
            if (path.Contains(current))
            {
                ...
                {
                    subErrors.Add(new ErrorCollection.Error(path[j].GetToken(), LocRM.GetString("ErrorText30")));
                }
                errors.Add(new ErrorCollection.Error(path[i].GetToken(), LocRM.GetString("ErrorText31"), true, subErrors.ToArray()));
            ...
            foreach (AALocalDecl localDecl in data.Locals[node])
            {
                ...
                while (block != null)
                {
                    foreach (AALocalDecl otherDecl in data.Locals[block])
                    {
                        ...
                        {
                            ErrorCollection.Error subError = new ErrorCollection.Error(otherDecl.GetName(), currentFile, LocRM.GetString("ErrorText32"));
                            errors.Add(new ErrorCollection.Error(localDecl.GetName(), currentFile, LocRM.GetString("ErrorText33"), false, subError));
        ...
        public override void CaseAALocalDecl(AALocalDecl node)
        {
            ...
            {//Cannot be both ref and out
                errors.Add(new ErrorCollection.Error(node.GetOut(), currentFile, LocRM.GetString("ErrorText34")));
                node.SetOut(null);
            }
        ...
        public override void CaseAFieldLvalue(AFieldLvalue node)
        {
            ...
            {
                errors.Add(new ErrorCollection.Error(node.GetName(), currentFile, LocRM.GetString("ErrorText35"), false));
        ...
        public override void CaseAStructFieldLvalue(AStructFieldLvalue node)
        {
            ...
            if (Util.GetAncestor<AMethodDecl>(node) == null)
            {
                errors.Add(new ErrorCollection.Error(node.GetName(), currentFile, LocRM.GetString("ErrorText36"), false));
                AAmbiguousNameLvalue newLvalue = new AAmbiguousNameLvalue(new ASimpleName(node.GetName()));
                node.ReplaceBy(newLvalue);
                newLvalue.Apply(this);
                return;
        ...
        public override void CaseASimpleInvokeExp(ASimpleInvokeExp node)
        {
            ...
            if (node.GetName().Text == "TriggerCreate" && node.GetArgs().Count == 1)
            {
                if (node.GetArgs()[0] is AStringConstExp)
                {
                    ...
                    {
                        errors.Add(new ErrorCollection.Error(node.GetName(), LocRM.GetString("ErrorText37"), true));
                    }
                    else if (matchingMethods.Count > 1)
                    {
                        ...
                        {
                            subErrors.Add(new ErrorCollection.Error(method.GetName(), LocRM.GetString("ErrorText38")));
                        }
                        errors.Add(new ErrorCollection.Error(node.GetName(), LocRM.GetString("ErrorText39"), true, subErrors.ToArray()));
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    for (int i = 0; i < triggers.Count; i++)
                    {
                        for (int j = i + 1; j < triggers.Count; j++)
                        {
                            ...
                            {
                                List<ErrorCollection.Error> subErrors = new List<ErrorCollection.Error>();
                                subErrors.Add(new ErrorCollection.Error(node.GetName(), currentFile, LocRM.GetString("ErrorText40")));
                                subErrors.Add(new ErrorCollection.Error(triggers[i].GetName(), Util.GetAncestor<AASourceFile>(triggers[i]), LocRM.GetString("ErrorText41") + triggers[i].GetName().Text));
                                subErrors.Add(new ErrorCollection.Error(triggers[i].GetName(), Util.GetAncestor<AASourceFile>(triggers[j]), LocRM.GetString("ErrorText41") + triggers[j].GetName().Text));

                                errors.Add(new ErrorCollection.Error(node.GetName(), currentFile, LocRM.GetString("ErrorText42"), false, subErrors.ToArray()), true);
        ...
        public override void CaseADynamicArrayType(ADynamicArrayType node)
        {
            ...
            if (newExp != null)
            {
                ...
                {
                    errors.Add(new ErrorCollection.Error(node.GetToken(), currentFile, LocRM.GetString("ErrorText43")));
                }
            }
