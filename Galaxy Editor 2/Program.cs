    ...
    static class Program
    {
        ...
        static void Main(string[] args)
        {
           /* {
            ...
            RegistryKey key = Registry.LocalMachine.OpenSubKey("SOFTWARE\\Microsoft\\XNA\\Framework\\v3.1", false);
            if (key == null)
            {
                XNAWarning dialog = new XNAWarning();
                if (dialog.ShowDialog() == DialogResult.OK)
                {
                    System.Diagnostics.Process.Start("http://www.microsoft.com/download/en/details.aspx?id=15163");
                    return;
                }
            }
            else
